...
public class VerCodeEditText extends androidx.appcompat.widget.AppCompatEditText implements
    ...
    private float mBottomLineHeight = 0;//底线的高度
    private float mBackgroundRadios = 0;//背景颜色的圆角
    private int mSelectedBackgroundColor;//选中的背景颜色
    private int mNormalBackgroundColor;//未选中的背景颜色
    ...
    private void initPaint() {
        ...
        mNormalBackgroundPaint.setColor(mNormalBackgroundColor);
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        mBottomLineHeight = ta.getDimension(R.styleable.VerCodeEditText_bottomLineHeight, 0);
        mBackgroundRadios = ta.getDimension(R.styleable.VerCodeEditText_backgroundRadios, 0);
        mSelectedBackgroundColor = ta.getColor(R.styleable.VerCodeEditText_selectedBackgroundColor,
                getColor(android.R.color.darker_gray));
        mNormalBackgroundColor = ta.getColor(R.styleable.VerCodeEditText_normalBackgroundColor,
                getColor(android.R.color.darker_gray));
        ta.recycle();
        ...
        for (int i = 0; i < mFigures; i++) {
            ...
            if (i == mCurrentPosition) {//选中的下一个状态
                if (mBackgroundRadios > 0 && Build.VERSION.SDK_INT > 21) {
                    canvas.drawRoundRect(start, 0, end, height, mBackgroundRadios, mBackgroundRadios, mSelectedBackgroundPaint);
                } else {
                    canvas.drawRect(start, 0, end, height, mSelectedBackgroundPaint);
                }
            } else {
                if (mBackgroundRadios > 0 && Build.VERSION.SDK_INT > 21) {
                    canvas.drawRoundRect(start, 0, end, height, mBackgroundRadios, mBackgroundRadios, mNormalBackgroundPaint);
                } else {
